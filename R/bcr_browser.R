...
.make_style_attrib <- function(css_attribs)
{
    stopifnot(is.character(css_attribs))
    paste0("style=\"", paste(css_attribs, collapse="; "), "\"")
}
...
.set_germline_background_color <- function(region, color)
{
    stopifnot(is.character(region), isSingleString(color))
    css_attribs <- c(sprintf("background: %s", color),
                     "font-weight: bold")
    span_tag <- sprintf("<span %s>", .make_style_attrib(css_attribs))
...
.make_VDJheadbox <- function(prefix, vdj_call, vdj_identity, bg_color)
{
    ...
    css_attribs <- c(sprintf("background: %s", bg_color),
                     "padding-left: 8px",
                     "padding-right: 8px",
                     "padding-bottom: 2px")
    span_tag <- sprintf("<span %s>", .make_style_attrib(css_attribs))
...
.make_header_line <- function(AIRR_df, Vcolor="#FFDDD2",
                                       Dcolor="#CFC",
                                       Jcolor="#CEF")
{
...
.make_VDJblocks <- function(AIRR_df,
                            Vcolor="#FFDDD2", Dcolor="#CFC", Jcolor="#CEF")
{
    ...
    L_width  <- v_sequence_start - 1L
    v_width  <- v_sequence_end - v_sequence_start + 1L
    vd_width <- d_sequence_start - v_sequence_end - 1L
    d_width  <- d_sequence_end - d_sequence_start + 1L
    dj_width <- j_sequence_start - d_sequence_end - 1L
    j_width  <- j_sequence_end - j_sequence_start + 1L
    R_width  <- nchar(sequence) - j_sequence_end

    mid_width1 <- vd_width + d_width + dj_width
    mid_width2 <- j_sequence_start - v_sequence_end - 1L
    mid_width <- ifelse(has_no_D, mid_width2, mid_width1)
    seq_width2 <- L_width + v_width + mid_width + j_width + R_width
    stopifnot(identical(nchar(sequence), seq_width2))
    ...
    v_segment <- .set_germline_background_color(v_segment, Vcolor)
    d_segment <- .set_germline_background_color(d_segment, Dcolor)
    j_segment <- .set_germline_background_color(j_segment, Jcolor)
    ...
    paste0(L_segment, v_segment, mid_segment, j_segment)
}
                           ...
                           Vcolor="#FFDDD2", Dcolor="#CFC", Jcolor="#CEF")
{
    ...
    L_segment  <- substr(sequence, 1L, v_sequence_start - 1L)
    v_segment  <- substr(sequence, v_sequence_start, v_sequence_end)
    vd_segment <- substr(sequence, v_sequence_end + 1L, d_sequence_start - 1L)
    d_segment  <- substr(sequence, d_sequence_start, d_sequence_end)
    dj_segment <- substr(sequence, d_sequence_end + 1L, j_sequence_start - 1L)
    j_segment  <- substr(sequence, j_sequence_start, j_sequence_end)
    R_segment  <- substr(sequence, j_sequence_end + 1L, nchar(sequence))
    vd_segment[has_no_D] <- d_segment[has_no_D] <- dj_segment[has_no_D] <- ""

    mid_segment1 <- paste0(vd_segment, d_segment, dj_segment)
    mid_segment2 <- substr(sequence, v_sequence_end + 1L, j_sequence_start - 1L)
    mid_segment <- ifelse(has_no_D, mid_segment2, mid_segment1)
    sequence2 <- paste0(L_segment, v_segment, mid_segment, j_segment, R_segment)
    stopifnot(identical(sequence, sequence2))
    ...
    mid_segment <- ifelse(has_no_D, mid_segment2, mid_segment1)
    paste0(L_segment, v_segment, mid_segment, j_segment, R_segment)
}
                        ...
                        CDRcolor="#EE4")
{
    ...
    header_line <- .make_header_line(AIRR_df, Vcolor=Vcolor,
                                              Dcolor=Dcolor,
                                              Jcolor=Jcolor)
    VDJblocks_line <- .make_VDJblocks(AIRR_df, Vcolor=Vcolor,
                                               Dcolor=Dcolor,
                                               Jcolor=Jcolor)
    ...
    FWR_CDR_line <- .make_FWR_CDR_line(AIRR_df, FWRcolor=FWRcolor,
                                                CDRcolor=CDRcolor)
    css_attribs <- c("border: 1px solid #BBB", "padding: 0px")
    td_tag <- sprintf("<td %s>", .make_style_attrib(css_attribs))
    html <- paste0(
        "<tr>", td_tag, "\n",
        header_line, "\n",
        "<pre style=\"margin: 5px; margin-top: 6px\">",
        VDJblocks_line, "\n",
        dna_line, "\n",
        sequence_aa_line, "\n",
        FWR_CDR_line,
        "</pre>\n</td></tr>\n")
    html <- c("<body><table style=\"border-collapse: collapse\">",
              html,
              "</table></body>\n")
