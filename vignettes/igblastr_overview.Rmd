...
- name: Ollivier Hyrien
  affiliation: Fred Hutch Cancer Center, Seattle, WA
date: "Compiled `r BiocStyle::doc_date()`; Modified 17 June 2026"
package: igblastr
vignette: |
...
IgBLAST-compatible _germline databases_ for human, mouse, and rhesus monkey
that were obtained from the OGRDB database (AIRR community). To get the list
of built-in _germline databases_, run:
```{r}
...
If the BCR sequences to analyze are from human, or one of the mouse strains
listed above, or rhesus monkey, then we can already select the appropriate
database with `use_germline_db()` and skip the subsection below.
...
is suitable (e.g. the sequences to analyze are not from human, mouse, or
rhesus monkey), then we can use `install_IMGT_germline_db()` to create and
install additional IgBLAST-compatible _germline databases_. Below, we show
how to create and install an IgBLAST-compatible _germline database_ from
germline V, D, and J gene sequences available on IMGT/V-QUEST download
site.
