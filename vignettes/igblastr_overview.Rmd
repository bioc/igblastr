...
- name: Ollivier Hyrien
  affiliation: Fred Hutch Cancer Center, Seattle, WA
date: "Compiled `r BiocStyle::doc_date()`; Modified 15 July 2026"
package: igblastr
vignette: |
...
```{r}
# count_suffixes() assumes suffixes of length 3, e.g. "_Hs"/"_Mm":
count_suffixes <- function(x) {nc <- nchar(x); table(factor(substr(x, nc-2, nc), levels=c("_Hs", "_Mm")))}

count_suffixes(AIRR_df$v_call)
count_suffixes(AIRR_df$d_call)
count_suffixes(AIRR_df$j_call)
```
