...
- name: Ollivier Hyrien
  affiliation: Fred Hutch Cancer Center, Seattle, WA
date: "Compiled `r BiocStyle::doc_date()`; Modified 15 April 2026"
package: igblastr
vignette: |
...
regions (FWR1–FWR4); and both nucleotide and protein-level alignments.
...
and J gene allele sequences for humans. Before `igblastn` can use them, these
germline gene allele sequences must first be stored in three separate BLAST
databases. We will refer to these as the V-region, D-region, and J-region
databases, respectively. When considered together, we will refer to them as
the _germline database_.
...
## Select germline database to use with `igblastn()`
...
germline V, D, and J gene allele sequences, the C-region sequences should
...
- Add facilities to retrieve germline databases from OGRDB, the AIRR
  Community’s Open Germline Receptor Database.
