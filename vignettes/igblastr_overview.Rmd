...
the Kymouse Antibody Database: <https://kydab.naturalantibody.com/>. More
precisely, we're going to analyze the Kymouse heavy-chain BCR sequences from
the [Influenza #1 Project](https://kydab.naturalantibody.com/influenza-01)
from KyDab.

### Prepare the query sequences

The 3,950 heavy-chain BCR sequences from this project are stored in
the `Influenza-KV7015_heavy.fasta` file included in `r Biocpkg("igblastr")`:
```{r}
...
BCR sequences align to human V, D, and J alleles:
```{r}
...
```
As expected, all Kymouse BCR sequences align to human V and J alleles.
