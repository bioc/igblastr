...
- name: Ollivier Hyrien
  affiliation: Fred Hutch Cancer Center, Seattle, WA
date: "Compiled `r BiocStyle::doc_date()`; Modified 18 June 2025"
package: igblastr
vignette: |
...
You can call `bcr_browser()` on `AIRR_df` to vizualize the annotated
sequences in the browser. For each sequence, the V, D, J, and C segments
will be shown as well as the FWR1-4 and CDR1-3 regions.

See `?igblastn` and `?bcr_browser` for more information.
...
See `?augment_germline_db` for more information.

Note that augmenting and restricting can be combined e.g. by pointing
`germline_db_V` to an augmented V germline database _and_ by supplying
the `germline_db_V_seqidlist` argument.
